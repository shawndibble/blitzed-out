  ...
  describe('Action file content structure', () => {
    it('all action files have consistent structure across languages', () => {
      ...
      [...baseLocalFiles, ...baseOnlineFiles].forEach(actionType => {
        gameModeFolders.forEach(mode => {
          if ((mode === 'local' && !baseLocalFiles.includes(actionType)) ||
          ...
          supportedLanguages.slice(1).forEach(lang => {
            ...
            if (baseContent.intensities && langContent.intensities) {
              expect(Object.keys(langContent.intensities).sort())
                .toEqual(Object.keys(baseContent.intensities).sort());
            }
          });
        });
      });
    ...
    it('intensity levels have consistent action counts', () => {
      const allActionTypes = [
        ...getActionFiles('en', 'local'),
        ...getActionFiles('en', 'online')
      ];
  ...
  describe('Translation quality checks', () => {
    it('no obviously untranslated content in non-English files', () => {
      const commonEnglishPhrases = [
        'click', 'press', 'hold', 'release', 'start', 'stop', 'begin', 'end',
        'take', 'put', 'place', 'remove', 'add', 'subtract', 'multiply',
        'seconds', 'minutes', 'times', 'for', 'until', 'while', 'then'
      ];

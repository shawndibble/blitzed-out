...
export default function latestMessageByType(messages: Message[], type: string): Message | undefined {
  return sortedMessages(messages).find((m) => m.type === type);
}

export function orderedMessagesByType(messages: Message[], type: string, order: 'ASC' | 'DESC' = 'ASC'): Message[] {
...
export function latestMessageBy(messages: Message[], callback: (message: Message) => boolean): Message | undefined {

...
- Anatomy placeholders: `{genital}` (dick/pussy), `{tip}` (tip/clit), `{hole}` (pussy/ass), `{chest}` (breasts/pecs). The token list lives once in `ANATOMY_PLACEHOLDERS` (`src/types/localPlayers.ts`) — every pattern, the `AnatomyPlaceholder` type, and the authoring chip list derive from it, so **no pattern edits**. A new token is that array plus, in all six locales: the term in `anatomy.json` (generic + 3 genders, and `straponTerms` if it strap-on swaps), the localized token name in `placeholders.json`, and a `customTiles.placeholderHelp.*` string in `translation.json` with its `ANATOMY_HELP_KEYS` entry. See `docs/engineering/features.md` § Placeholders.

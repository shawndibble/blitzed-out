...
- `MIGRATION_VERSION` (`src/services/migration/constants.ts`) no longer forces a reseed on bump — the version-change orchestration that once cleared status on upgrade had no live caller and was removed. A version bump today is documentation only. `isCurrentLanguageMigrationCompleted` (`statusManager.ts`) short-circuits on `completedLanguages.includes(locale)` and never compares the stored `version`.
  > ⚠️ **Consequence for anyone planning a data fix: seeding cannot reach existing devices.** A change that only writes new data at seed time will type-check, pass tests, and land on fresh installs only — every current player keeps the old state indefinitely. Verify reachability _before_ choosing a seed-layer fix. Options are (a) read at display time instead — see [ADR-0004](../adr/0004-presentation-only-content-read-from-bundle.md), (b) a one-off backfill that runs independently of the seed gate, or (c) rework the gate to honour the version, accepting that every device re-seeds.

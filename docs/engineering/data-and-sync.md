...
- **Realtime Database** — presence and WebRTC signaling:
  - `users/{uid}` — presence (`displayName`, `room`, `isAnonymous`, `lastSeen`). Owned end-to-end (write + read) by `src/services/roomPresence.ts`. The read is **not room-scoped**: `getUserList` holds `onValue` on the whole `users` node and filters by room in the browser, unconditionally for every client in a room (`UserListProvider`, mounted per route). So any presence write by any user anywhere re-fires the callback with the entire node and re-scans it, and both room-entry cost and steady-state cost scale with the platform-wide row count rather than the room's. `cleanupStaleUsers` is therefore the only thing holding that cost down — when it silently stalled, the node reached 1156 rows. A room-scoped `orderByChild('room').equalTo(...)` would remove the coupling — `room` is uppercased on both write and read, so the server returns exactly the set the browser filter builds. It needs `.indexOn: ["lastSeen", "room"]`; dropping `lastSeen` there re-breaks the sweep.
  - `video-calls/{roomId}/{users,offers,answers,ice-candidates}` — WebRTC signaling. A roster entry is `{joinedAt, lastSeen, status}` plus the published media flags `cam` (`on`/`off`/`none`/`hidden`) and `mic` (`on`/`off`); both flags are optional, and absent means unknown rather than off, so clients on older builds stay readable. The `users` node has **two readers**: `videoCallStore`, once you join, and `callPresenceStore` — a **read-only** listener mounted for everyone in the room (via `CallPresenceWatcher`, over the `services/callPresence.ts` seam) that powers the participant badge. The passive one claims no slot and opens no camera, which is what lets the count be visible before joining; `.read` here was already `auth != null`, so it needed no rules change. The two judge staleness by different windows on purpose, and the join gate uses the dialling window rather than the badge's — see `features.md` § In-room video calling. Neither reader may tear down with `off(ref)`: that detaches _both_.

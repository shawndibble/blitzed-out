...
- **Source maps / telemetry:** `@sentry/vite-plugin` is registered **only** when `SENTRY_UPLOAD_SOURCEMAPS=true` (`vite.config.ts`). Nothing sets it, so production stacks are minified. The auth token is in `.env.sentry-build-plugin` (git-ignored); turning the flag on is all that's outstanding.

...
- **Participant badge (`callPresenceStore` + `VideoCallProvider`):** a badge on the desktop camera icon and the mobile video tab shows how many people are on the call, so someone who has not joined can see there is one to join. It cannot come from `videoCallStore` — desktop subscribes to nothing until the sidebar opens, mobile not until the Call button is tapped, and on desktop _opening the panel is joining_. So `VideoCallProvider` (which used to auto-join, and was inert only because of where it was mounted) now hosts a **read-only** listener on `video-calls/{roomId}/users`: no slot claimed, no camera opened. `.read` on that node was already `auth != null`, so no rules change. The count is **total live participants including yourself** — a lone caller showing "1" is what advertises the call to the room — hidden at 0, and identical whether you are in the call or out of it.
- **Two freshness windows, on purpose:** the badge counts by `PRESENCE_STALE_MS` (2 min), not the 10-minute `ROSTER_STALE_MS` that governs dialling. A ghost only costs a mesh slot, but a badge is a number people act on, so being wrong for ten minutes after a crash would send them into an empty call. `liveRoster(users, now, staleMs)` takes the window as a parameter — same rule, different tolerance. Because `onValue` does not fire when nothing changes and staleness is judged against the clock, the store also re-derives the count on a 30s tick, and skips the state write when the number has not moved (every participant's heartbeat rewrites `lastSeen` every 30s while the count sits still).
- **Capacity alert & the full-call gate:** one `Alert` at the top of `VideoCallPanel` (`components/VideoCall/CallCapacityAlert`) warns at `CALL_QUALITY_WARNING_PARTICIPANTS` (4) and escalates to "call full" at the cap — one component with two states, so the thresholds cannot drift into both rendering at once. It reads the **passive presence store, never `videoCallStore`**: a refused joiner early-returns from `initialize()`, so store state is empty for exactly the person who needs to be told. The over-cap check sits at the **top of `initialize()`, before `getUserMedia`** — by the time `claim()` runs the stream already exists, so a check at the claim site would still have prompted for the camera and lit the device up for someone who can never be dialled. Top-of-initialize also covers both call sites at once. It reads the same count the badge shows, so the two never disagree. Deliberately not guarded: two people passing the check at the same instant can land the roster at 7, and an unloaded count fails open rather than putting an RTDB round trip between the tap and the camera on every join.

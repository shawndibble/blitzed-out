...
- **TURN relay credentials.** The primary path is now short-lived: `getTurnCredentials` (a callable requiring `context.auth`) mints Cloudflare credentials server-side from the `CLOUDFLARE_TURN_TOKEN` Secret Manager secret, which never reaches the client. The **fallback** credentials (`VITE_METERED_*`) still ship in the bundle and remain harvestable — treat as low-privilege and **rotate periodically**. They are reached only when minting fails.
